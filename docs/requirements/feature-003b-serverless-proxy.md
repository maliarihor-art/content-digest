# Feature 003b — Serverless AI proxy (M2)

## User story
As the app, I want a stateless serverless function that holds the AI provider key
...
- **Provider: Google Gemini, free tier, over `fetch`** (ADR 004) — no SDK, zero
  runtime dependencies. Key `GEMINI_API_KEY`.
- **Reuse the M1 seam:** `buildDigestPrompt`, `DIGEST_MODEL`, `DIGEST_MAX_TOKENS`,
  and `parseDigestResponse` — do **not** rebuild prompt or validation. The seam is
  provider-neutral; the proxy maps the request onto Gemini's REST body.
- **Pure core, thin adapter.** All logic lives in a pure, unit-tested module
  `app/src/digest/ai/service.ts` (`runDigest(body, call)`); the actual AI call is
  **injected** as a `DigestCaller`. The Vercel handler `api/digest.ts` (repo root)
  only reads `process.env.GEMINI_API_KEY`, calls Gemini's `generateContent` with
  `fetch` (forcing `responseMimeType: "application/json"`), and forwards the
  core's HTTP result.
...
### runDigest (pure core)
- GIVEN a body `{ text: "<non-empty article>" }` and a `DigestCaller` that returns
...
- GIVEN the `DigestCaller` rejects (network / non-OK HTTP)
  WHEN `runDigest` runs
  THEN it resolves to `{ status: 502, body: { error } }` and never throws.
- GIVEN the `DigestCaller` resolves with malformed JSON / a non-`Digest` reply
  WHEN `runDigest` runs
...
- The handler reads the key only from `process.env.GEMINI_API_KEY`; a missing key
  surfaces as a `502` via the caller (never a `200`).
