...
- **T1.4 — M3: Frontend integration + fallback.** On "Add digest": `POST /api/digest`; on success use the returned `Digest`; on network/error/rate-limit/missing-key, fall back to local `buildDigest(text)`. Add loading + error UI states. Keep determinism note: AI results vary, that's expected.
